...
public:
	...
	void reset(dtPolyRef ref, const double* pos);
	...
	int findCorners(double* cornerVerts, unsigned char* cornerFlags,
	...
	///  @param[in]		navquery				The query object used to build the corridor.
	///  @param[in]		filter					The filter to apply to the operation.			
	void optimizePathVisibility(const double* next, const double pathOptimizationRange,
								dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	...
	bool fixPathStart(dtPolyRef safeRef, const double* safePos);

	bool trimInvalidPath(dtPolyRef safeRef, const double* safePos,
						 dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	...
	bool movePosition(const double* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	...
	///  @param[in]		filter		The filter to apply to the operation.
	/// @return Returns true if move succeeded.
	bool moveTargetPosition(const double* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	
	/// Loads a new path and target into the corridor.
	///  @param[in]		target		The target location within the last polygon of the path. [(x, y, z)]
	///  @param[in]		path		The path corridor. [(polyRef) * @p npolys]
	///  @param[in]		npath		The number of polygons in the path.
	void setCorridor(const double* target, const dtPolyRef* polys, const int npath);
	...
	inline const double* getTarget() const { return m_target; }
