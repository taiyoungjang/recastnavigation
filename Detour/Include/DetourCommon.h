...
inline void dtVcross(double* dest, const double* v1, const double* v2)
{
...
inline double dtVdot(const double* v1, const double* v2)
{
...
inline void dtVmad(double* dest, const double* v1, const double* v2, const double s)
{
	dest[0] = v1[0]+v2[0]*s;
	dest[1] = v1[1]+v2[1]*s;
	dest[2] = v1[2]+v2[2]*s;
...
inline void dtVlerp(double* dest, const double* v1, const double* v2, const double t)
{
...
inline void dtVadd(double* dest, const double* v1, const double* v2)
{
	dest[0] = v1[0]+v2[0];
	dest[1] = v1[1]+v2[1];
	dest[2] = v1[2]+v2[2];
...
inline void dtVsub(double* dest, const double* v1, const double* v2)
{
	dest[0] = v1[0]-v2[0];
	dest[1] = v1[1]-v2[1];
	dest[2] = v1[2]-v2[2];
...
inline void dtVscale(double* dest, const double* v, const double t)
{
	dest[0] = v[0]*t;
	dest[1] = v[1]*t;
	dest[2] = v[2]*t;
...
inline void dtVmin(double* mn, const double* v)
{
	mn[0] = dtMin(mn[0], v[0]);
	mn[1] = dtMin(mn[1], v[1]);
	mn[2] = dtMin(mn[2], v[2]);
...
///  @param[in]		v	A vector. [(x, y, z)]
inline void dtVmax(double* mx, const double* v)
{
	mx[0] = dtMax(mx[0], v[0]);
	mx[1] = dtMax(mx[1], v[1]);
	mx[2] = dtMax(mx[2], v[2]);
...
inline void dtVset(double* dest, const double x, const double y, const double z)
{
...
inline void dtVcopy(double* dest, const double* a)
{
	dest[0] = a[0];
	dest[1] = a[1];
	dest[2] = a[2];
...
inline double dtVlen(const double* v)
{
...
inline double dtVlenSqr(const double* v)
{
...
inline double dtVdist(const double* v1, const double* v2)
{
...
inline double dtVdistSqr(const double* v1, const double* v2)
{
	const double dx = v2[0] - v1[0];
	const double dy = v2[1] - v1[1];
	const double dz = v2[2] - v1[2];
...
inline double dtVdist2D(const double* v1, const double* v2)
{
	const double dx = v2[0] - v1[0];
	const double dz = v2[2] - v1[2];
...
inline double dtVdist2DSqr(const double* v1, const double* v2)
{
	const double dx = v2[0] - v1[0];
	const double dz = v2[2] - v1[2];
	return dx*dx + dz*dz;
...
inline void dtVnormalize(double* v)
{
	double d = 1.0 / dtMathSqrtf(dtSqr(v[0]) + dtSqr(v[1]) + dtSqr(v[2]));
	v[0] *= d;
	v[1] *= d;
	v[2] *= d;
}
...
inline bool dtVequal(const double* p0, const double* p1)
{
	static const double thr = dtSqr(1.0/16384.0);
	const double d = dtVdistSqr(p0, p1);
	return d < thr;
}
...
inline bool dtVisfinite(const double* v)
{
	bool result =
		dtMathIsfinite(v[0]) &&
		dtMathIsfinite(v[1]) &&
...
///  @param[in]		v	A point. [(x, y, z)]
inline bool dtVisfinite2D(const double* v)
{
...
///
/// The vectors are projected onto the xz-plane, so the y-values are ignored.
inline double dtVdot2D(const double* u, const double* v)
{
...
///
/// The vectors are projected onto the xz-plane, so the y-values are ignored.
inline double dtVperp2D(const double* u, const double* v)
{
...
inline double dtTriArea2D(const double* a, const double* b, const double* c)
{
...
inline bool dtOverlapBounds(const double* amin, const double* amax,
...
@fn double dtTriArea2D(const double* a, const double* b, const double* c)
@par
