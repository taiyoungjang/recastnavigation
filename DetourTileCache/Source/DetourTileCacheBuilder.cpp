...
dtStatus dtMarkBoxArea(dtTileCacheLayer& layer, const double* orig, const double cs, const double ch,
					   const double* bmin, const double* bmax, const unsigned char areaId)
{
	const int w = (int)layer.header->width;
	const int h = (int)layer.header->height;
	const double ics = 1.0/cs;
	...
	int minx = (int)floor((bmin[0]-orig[0])*ics);
	int miny = (int)floor((bmin[1]-orig[1])*ich);
	int minz = (int)floor((bmin[2]-orig[2])*ics);
	int maxx = (int)floor((bmax[0]-orig[0])*ics);
	int maxy = (int)floor((bmax[1]-orig[1])*ich);
	int maxz = (int)floor((bmax[2]-orig[2])*ics);
...
dtStatus dtMarkBoxArea(dtTileCacheLayer& layer, const double* orig, const double cs, const double ch,
					   const double* center, const double* halfExtents, const double* rotAux, const unsigned char areaId)
{
	const int w = (int)layer.header->width;
	const int h = (int)layer.header->height;
	const double ics = 1.0/cs;
	...
	double cx = (center[0] - orig[0])*ics;
	double cz = (center[2] - orig[2])*ics;
	
	double maxr = 1.41*dtMax(halfExtents[0], halfExtents[2]);
	int minx = (int)floor(cx - maxr*ics);
	int maxx = (int)floor(cx + maxr*ics);
	int minz = (int)floor(cz - maxr*ics);
	int maxz = (int)floor(cz + maxr*ics);
	int miny = (int)floor((center[1]-halfExtents[1]-orig[1])*ich);
	int maxy = (int)floor((center[1]+halfExtents[1]-orig[1])*ich);
	...
	double xhalf = halfExtents[0]*ics + 0.5;
	double zhalf = halfExtents[2]*ics + 0.5;

	for (int z = minz; z <= maxz; ++z)
	{
		for (int x = minx; x <= maxx; ++x)
		{			
			double x2 = 2.0*(double(x) - cx);
			double z2 = 2.0*(double(z) - cz);
			double xrot = rotAux[1]*x2 + rotAux[0]*z2;
			if (xrot > xhalf || xrot < -xhalf)
				continue;
			double zrot = rotAux[1]*z2 - rotAux[0]*x2;
